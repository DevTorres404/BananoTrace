...
DROP TRIGGER IF EXISTS "trg_generar_codigo_finca" ON "fincas";
DROP FUNCTION IF EXISTS "generar_codigo_finca"();
DROP TRIGGER IF EXISTS "trg_generar_codigo_unidad" ON "unidades_trazables";
DROP FUNCTION IF EXISTS "generar_codigo_unidad"();
DROP TRIGGER IF EXISTS "trg_sync_codigo_lote" ON "lotes_produccion";
DROP FUNCTION IF EXISTS "sync_codigo_lote"();
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'unidades_trazables' AND (NEW."codigo" IS NULL OR BTRIM(NEW."codigo") = '') THEN
